...
export default function Modal({ isOpen, onClose, children }: ModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 transition-opacity duration-700"
      onClick={onClose} // Close modal when clicking outside
    >
      <div 
        ref={modalRef}
        className={`bg-white rounded-lg shadow-lg max-w-lg w-full mx-4 sm:mx-0 transition-opacity duration-300 ease-in-out ${isAnimating ? 'opacity-100' : 'opacity-0'}`} 
